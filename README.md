...
**最快：下載執行檔（免裝 Python）** — 到 [Releases](../../releases) 抓對應平台單檔：
`cc-codex-sync-macos-arm64`（Apple Silicon）／`cc-codex-sync-macos-intel`／`cc-codex-sync-windows-x64.exe`。
雙擊（或終端機直接執行）會出互動選單，第一次選「初次設定」即完成三步上手；日常選「一鍵同步」。
執行檔模式的資料目錄在 `~/.cc-codex-sync/`（macOS 首次執行若被 Gatekeeper 擋：右鍵→打開；
...
```bash
python3 memsync/cli.py setup                   # 三步上手（建對照檔→掃描→首次同步），或手動：
